...
export async function PUT(request: NextRequest) {
  ...
  const template = process.env.API_SETTINGS_BANNER_PUT_URL_TEMPLATE; // e.g., /v1/api/settings/homepage/banner
  const candidates: string[] = [];
  if (template) candidates.push(template);
  candidates.push("/v1/api/settings/homepage/banner");
  candidates.push("/v1/api/site-settings/homepage/banner");
