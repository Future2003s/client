...
export async function GET(request: NextRequest) {
  ...
  const template = process.env.API_SETTINGS_HOMEPAGE_GET_URL_TEMPLATE; // e.g., /v1/api/settings/homepage
  const candidates: string[] = [];
  if (template) candidates.push(template);
  candidates.push("/v1/api/settings/homepage");
  candidates.push("/v1/api/site-settings/homepage");
...
export async function PUT(request: NextRequest) {
  const base = envConfig.NEXT_PUBLIC_API_END_POINT;
  const template = process.env.API_SETTINGS_HOMEPAGE_PUT_URL_TEMPLATE; // e.g., /v1/api/settings/homepage
  const candidates: string[] = [];
  if (template) candidates.push(template);
  candidates.push("/v1/api/settings/homepage");
  candidates.push("/v1/api/site-settings/homepage");
